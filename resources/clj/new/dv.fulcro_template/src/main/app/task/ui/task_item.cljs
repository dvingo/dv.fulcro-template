...
(defsc TaskItem
  ...
  {:query (fn [] (conj task/fulcro-query :ui/show-debug?))
   :ident ::task/id}
  [:div.ui.segment
...
(defn empty-form [] (task/make-task {::task/description ""}))

(defn task-valid [form field]
  (let [v (get form field)]
    (when (malli/-reference? field)
      (malli/validate field v))))

(def validator (fs/make-validator task-valid))

(defsc TaskItemReturn [this props]
  {:query (fn [] (conj task/fulcro-query :server/message :server/error?))
...
(defsc TaskForm
  ...
  {:query             (fn [] (conj task/fulcro-query  fs/form-config-join
   ...
   :initial-state     (fn [_] (fs/add-form-config
                                TaskForm (merge (empty-form) {:ui/show-form-debug? false})))
      ...
      [:div.ui.grid
       [:div.column.four.wide>button
        {:tabIndex 0
         ...
         :onClick
         (fu/prevent-default
           #(let [task (task/fresh-task props)]
              (fmachine/submit-entity! this
                {:entity          task
                 :machine         ::form-machine
                 :creating?       true
                 :remote-mutation 'pro.kala-app.weather.task/create-task
